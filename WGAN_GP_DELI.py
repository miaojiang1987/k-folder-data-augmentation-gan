...
class WGAN_GP(object):
    def __init__(self, args):
        ...
        params = list(self.G.parameters()) + list([self.sigma,self.mu])
        self.G_optimizer = optim.Adam(params, lr=args.lrG, betas=(args.beta1, args.beta2))
    ...
    def train(self):
        ...
        for epoch in range(self.epoch):
            ...
            for iter, (x_, _) in enumerate(self.data_loader):
                ...
                #epsilon = torch.FloatTensor(self.batch_size, self.z_dim, 1, 1).normal_(0, 1).cuda()
                #noise =self.mu+self.sigma*z_
                #noise.cuda()
                G_ = self.G(z_)
                D_fake = self.D(G_)
                ...
                if ((iter+1) % self.n_critic) == 0:
                    ...
                    #noise.cuda()
                    G_ = self.G(z_)
                    #sig_loss = 0.5 *(abs(1 - self.sigma)).mean()
                    #mu_loss= 0.5* (abs(0.1-self.mu)).mean()
                    D_fake = self.D(G_)
                    G_loss = -torch.mean(D_fake)#-sig_loss-mu_loss
                    G_loss.backward()
